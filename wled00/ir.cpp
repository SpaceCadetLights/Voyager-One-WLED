...
//----------------------------------Space Cadets Remote Switch Control-------------------------------------

void decodeIR44(uint32_t code)
{
  switch (code) {
    case IR44_OFF       : changeBrightness(10);  Serial.println(bri);                          break; //Bright Up
    case IR44_BLUE      : changeBrightness(-10);                                               break; //Bright Down
    case IR44_WHITE         : briLast = bri; bri = 0;                                          break; //Light Off
    case IR44_ON          : bri = briLast;                                                     break; //Light On
    case IR44_BPLUS      : changeEffectIntensity( 16);                                         break; //Intensity up
    case IR44_RED        : changeEffectIntensity(-16);                                         break; //Intensity Down
    case IR44_BMINUS       : changeEffectSpeed( 16);                                           break; //Speed up
    case IR44_GREEN        : changeEffectSpeed(-16);                                           break; //Speed Down
    
//-----Solid Color and pallet Selections-----
    case IR44_REDDISH       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; effectCurrent = 12;  effectPalette = 0;                         break; //Fade Button
    case IR44_GREENISH      : AutoAudioEffectChanger = false; presetCyclingEnabled = false; effectCurrent = 92;  effectPalette = 0;  effectIntensity = 200;  effectSpeed = 190;  break; //Breathe Button
    case IR44_DEEPBLUE      : AutoAudioEffectChanger = false; presetCyclingEnabled = false; effectCurrent = 23;  effectPalette = 0;   effectIntensity = 100;  effectSpeed = 254;  break; //Strobe Button
    case IR44_ORANGE         : AutoAudioEffectChanger = false; presetCyclingEnabled = false; colorFromUint24(COLOR_RED); effectCurrent = 0;          break;
    case IR44_TURQUOISE      : AutoAudioEffectChanger = false; presetCyclingEnabled = false; colorFromUint24(COLOR_ORANGE); effectCurrent = 0;       break;
    case IR44_PURPLE   : AutoAudioEffectChanger = false; presetCyclingEnabled = false; colorFromUint24(COLOR_YELLOWISH); effectCurrent = 0;          break;
    case IR44_WARMWHITE        : AutoAudioEffectChanger = false; presetCyclingEnabled = false; colorFromUint24(COLOR_GREEN); effectCurrent = 0;      break;
    case IR44_YELLOWISH        : AutoAudioEffectChanger = false; presetCyclingEnabled = false; colorFromUint24(COLOR_CYAN); effectCurrent = 0;                  break;
    case IR44_CYAN       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; colorFromUint24(COLOR_BLUE); effectCurrent = 0;                        break;
    case IR44_MAGENTA     : AutoAudioEffectChanger = false; presetCyclingEnabled = false; colorFromUint24(COLOR_MAGENTA); effectCurrent = 0;                    break;
    case IR44_COLDWHITE        : AutoAudioEffectChanger = false; presetCyclingEnabled = false; colorFromUint24(COLOR_PINK); effectCurrent = 0;                  break;
    case IR44_WARMWHITE2       : AutoAudioEffectChanger = false; presetCyclingEnabled = false;{ 
      if (useRGBW) {
        if (col[3] > 0) col[3] = 0; 
        else {              colorFromUint32(COLOR2_NEUTRALWHITE); effectCurrent = 0; }
      } else                colorFromUint24(COLOR_NEUTRALWHITE);  effectCurrent = 0;  }  break;
      
//-----Standard Effect Buttons-----
   case IR44_YELLOW     : AutoAudioEffectChanger = false; if (presetCyclingEnabled) {presetCyclingEnabled = false;} else if (!presetCyclingEnabled) {presetCyclingEnabled = true;}   break; //Auto Button
   case IR44_AQUA       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(1)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 1 (rainbow)
   case IR44_PINK       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(2)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 2
   case IR44_COLDWHITE2 : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(3)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 3
   case IR44_REDPLUS    : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(4)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 4
   case IR44_GREENPLUS  : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(5)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 5
   case IR44_BLUEPLUS   : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(6)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 6
   case IR44_QUICK      : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(7)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 7
   case IR44_REDMINUS   : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(8)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 8
   case IR44_GREENMINUS : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(9)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 9
   case IR44_BLUEMINUS  : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(10)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 10
   case IR44_SLOW       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(11)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 11
   case IR44_DIY1       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(12)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 12
   case IR44_DIY2       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(13)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 13
   case IR44_DIY3       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(14)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 14
   case IR44_AUTO       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; if (!applyPreset(15)) { effectCurrent = FX_MODE_STATIC;  effectPalette = 0; } break; //Button 15

//-----Audio Reactive Effect Buttons----
   case IR44_DIY4 :
   presetCyclingEnabled = false;
   AutoAudioEffectChanger = true;
   
   Serial.println("Auto Changer Enabled");
   //effectPalette = 1; 
   break; //Audio Button Auto
   
   case IR44_DIY5       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; effectCurrent = 119;    effectPalette = 1; effectSpeed = 210;        break; //Audio Button 1
   case IR44_DIY6       : AutoAudioEffectChanger = false; presetCyclingEnabled = false; effectCurrent = 122;    effectPalette = 1; effectSpeed = 210;        break; //Audio Button 2
   case IR44_FLASH      : AutoAudioEffectChanger = false; presetCyclingEnabled = false; effectCurrent = 124;    effectPalette = 1; effectSpeed = 210;        break; //Audio Button 3
   case IR44_JUMP3      : AutoAudioEffectChanger = false; presetCyclingEnabled = false; effectCurrent = 125;    effectPalette = 1; effectSpeed = 210;        break; //Audio Button 4
   case IR44_JUMP7      : AutoAudioEffectChanger = false; presetCyclingEnabled = false; effectCurrent = 113;    effectPalette = 1; effectSpeed = 210;        break; //Audio Button 5
   case IR44_FADE3      : AutoAudioEffectChanger = false; presetCyclingEnabled = false; effectCurrent = 131;    effectPalette = 1; effectSpeed = 210;        break; //Audio Button 6  used to be 114
   case IR44_FADE7      : AutoAudioEffectChanger = false; presetCyclingEnabled = false; effectCurrent = 116;    effectPalette = 1; effectSpeed = 210;        break; //Audio Button 7
...
//----------------END of Space Cadets Remote Decode----------------
