...
if (AutoAudioEffectChanger) {
      ...
      effectSpeed = 210;  //Change the speed to higher when in audio reactive mode
    colorUpdated(1);

    if (millis()-lastChangeTime > changeDelayMs) {
    lastChangeTime = millis();

    nextDisplayedEffect = (effectCurrent + 1);
    if (nextDisplayedEffect > 131){
      nextDisplayedEffect = 112;
    } else if (nextDisplayedEffect < 112) {
      nextDisplayedEffect = 112;
    }
    Serial.print("Auto Pattern Changed To ");
    Serial.println(nextDisplayedEffect);
    effectCurrent = nextDisplayedEffect; //Changes the effect based upon the incremental change 
    colorUpdated(6);
  }//end of every n seconds
  
  
}

// else if (!AutoAudioEffectChanger) {
//effectCurrent = 122;
//  
//}
